...
export function Rating({
  ...
  async function rate(rating:number){
    ...
    if(response.ok){
      setCount(data.count);
      setTotal(data.total);
      setMessage(`Rated ${rating}: ${labels[rating-1]}`);
      setOpen(false);
    }else{
      setMessage(data.error||'The rating could not be saved.');
    }
  ...
  return (
    <section className="rating" aria-label="Story rating">
      <div className="rating-summary">
        Rated: <strong>{count}</strong> {count===1?'time':'times'}
        <span> · </span>
      ...
      {message&&
        <p className="rating-message" aria-live="polite">
          {message}
        </p>
      }
    </section>
