...
export async function deleteApprovedStory(formData:FormData){
  await requireOwner();const id=String(formData.get('id'));const db=adminDb();
  const {data:story,error}=await db.from('stories').select('slug,submission_id').eq('id',id).single();if(error||!story)throw new Error('The accepted submission could not be found.');
  if(story.submission_id){const {error:logError}=await db.from('decision_email_log').delete().eq('submission_id',story.submission_id);if(logError)throw logError;}
  const {error:storyError}=await db.from('stories').delete().eq('id',id);if(storyError)throw storyError;
  if(story.submission_id){const {error:submissionError}=await db.from('submissions').delete().eq('id',story.submission_id);if(submissionError)throw submissionError;}
  refresh(story.slug);
}
